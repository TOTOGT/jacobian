...
theorem jacobian_det_eq_neg_two :
  ...
  simp only [det_fin_three, pderiv_add, pderiv_mul, pderiv_pow, pderiv_X, pderiv_C,
             Fin.zero_eta, Fin.isValue, Pi.single_eq_same, Pi.single_eq_of_ne]
  ring
...
theorem points_collide :
  ...
  refine ⟨?_, ?_, ?_⟩ <;>
  · unfold F1 F2 F3 xVar yVar zVar p1 p2 p3
    simp only [eval_add, eval_mul, eval_pow, eval_C, eval_X, Matrix.cons_val_zero,
...
theorem not_injective_despite_constant_jacobian :
  ...
  have h_eval : (eval p1 F1, eval p1 F2, eval p1 F3) = (eval p2 F1, eval p2 F2, eval p2 F3) := by
    rw [e1, e2]
  have h12 : p1 = p2 := hinj h_eval
